...
void Join::cmd_join(std::string &channel_name, std::string &key, Client& client)
{
...
void Join::execute(Client& client, Arguments args)
{
    ...
    if(args.size() == 1)
    {
        for (size_t j = 0; j < join_channels.size(); j++)
            cmd_join(join_channels[j], empty, client);
        return ;
    }
    ...
    {
        if ( j  < join_keys.size())
            cmd_join(join_channels[j], join_keys[j], client);
        else
            cmd_join(join_channels[j], empty, client);
    }
}
